        ...
        if (state == 1) {
            if (turn) {
                if (elem(pos.asString + "e").innerHTML == " ") {
                    sendRequest(`../functions/record_hit.php?id=${gameId}&playerId=${playerId}&pos=${pos.asString}`)
                        .then((hitData) => {
                            if (hitData.success) {
...
elem("confirm").onclick = function () {
    if (placestate == 1) {
        ...
        elem("confirm").disabled = true;
        elem("cancel").disabled = true;
        sendRequest(`../functions/set_ship_placement.php?id=${gameId}&playerId=${playerId}&up=${isUp ? "1" : "0"}&pos=${shipStart.starting.asString}&length=${getShipLength(shipsToPlace)}`)
            .then((placeData) => {
                if (placeData.success) {
                    ...
                    if (shipsToPlace == 0) {
                        sendRequest(`../functions/complete_ship_placement.php?id=${gameId}&playerId=${playerId}`)
                            .then((doneData) => {
                                if (doneData.success) {
                                    if (doneData.waitforenemy) {
                                        ...
                                        state = 1;
                                        sendRequest("../functions/fetch_enemy_board.php?id=" + gameId + "&playerId=" + playerId).then((enemyBoardData) => {
...
sendRequest("../functions/fetch_game_board.php?id=" + gameId + "&playerId=" + playerId).then((boardInfo) => {
...
setInterval(() => {
    ...
    if (state == 1) {
        ...
        sendRequest("../functions/fetch_turn_state.php?id=" + gameId + "&playerId=" + playerId).then((turnData) => {
            if (turnData.success) {
        ...
        sendRequest("../functions/fetch_game_board.php?id=" + gameId + "&playerId=" + playerId).then((boardData) => {
        ...
        sendRequest("../functions/fetch_game_state.php?id=" + gameId + "&playerId=" + playerId).then((stateData) => {
        ...
        sendRequest("../functions/retrieve_stats.php?id=" + gameId + "&playerId=" + playerId).then((statsData) => {
    ...
    if (waitingforopponent) {
        ...
        sendRequest("../functions/opponent_finished_turn.php?id=" + gameId + "&playerId=" + playerId).then((data) => {
